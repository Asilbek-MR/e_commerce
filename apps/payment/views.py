...
class generateTokenView(APIView):
    def get(self, request, format=None):
        try:
            ...
            return Response({'error': 'Something went wrong when retrieving braintree token'},status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        ...
        try:
            ...
            for cart_item in cart_items:
                if not Product.objects.filter(id=cart_item.product.id).exists():
                    return Response({'error':'A product ID provided does not exist'},status=status.HTTP_404_NOT_FOUND)
...
class ProcessPaymentView(APIView):
    def post(self, request,format=None):
        ...
        try:
            # Create transaction con braintree
            ...
            return Response({'error':'Error processing the transaction'},status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        ...
        if newTransaction.is_success or newTransaction.transaction:
            ...
            try:
                ...
                return Response({'error':'Transaction succeeded and order created,but failed to create an order item'},status=status.HTTP_500_INTERNAL_SERVER_ERROR)
            ...
            for cart_item in cart_items:
                try:
                    ...
                    return Response({'error': 'Transaction succeeded and order created, but failed to create an order item'},
