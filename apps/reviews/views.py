...
class GetProductReviewViews(APIView):
    def get(self,request,productId,format=None):
        ...
        try:
            ...
            if Review.objects.filter(user=user,product=product).exists():
                ...
                results['comment']=review.comment
                results['date_created']=review.date_created
                results['user']=review.user.first_name
            return Response({'review':results},status=status.HTTP_200_OK)
...
class CreateProductReviewView(APIView):
    def post(self,request,productId,format=None):
        ...
        try:
            ...
            return Response({'error':'This pass a comment when creating review'},status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            if Review.objects.filter(user=user,product=product).exists():
                ...
                for review in reviews:
                    ...
                    results.append(item)
            return Response({'review':result,'reviews':results},status=status.HTTP_201_CREATED)
        except:
...
class UpdateProductReviewView(APIView):
    def put(self,request,productId, format=None):
        ...
        try:
            ...
            return Response({'error':'Rating must be decimal value'},status=status.HTTP_400_BAD_REQUEST)
        try:
            comment=str(data['comment'])
        except:
        ...
        try:
            ...
            return Response({'review':result,'reviews':results},status=status.HTTP_200_OK)
        
        except:
            return Response({'error':'Something went wrong when update review'},status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        ...
        try:
            ...
            if Review.objects.filter(product=product).exists():
                if rating ==0.5:
                    ...
                    reviews=Review.objects.order_by('-date_created').filter(rating__lte=rating,product=product).filter(
                        rating__gte=(rating - 0.5),product=product
                    )
                for review in reviews:
                    item={}
