    ...
    def post(self, request,format=None):
        ...
        try:
            if not Product.objects.filter(id=product_id).exists():
                return Response({'error':'This product does nor exist'},status=status.HTTP_404_NOT_FOUND)
...
class GetItemTotalView(APIView):
    def get(self, request,format=None):
        ...
        try:
            ...
            return Response({"error":'Something went wrong when retrieving total number of wishlist items'},status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class RemoveItemView(APIView):
    def delete(self, request,format=None):
        ...
        try:
            wishlist=WishList.objects.get(user=user)
            if not Product.objects.filter(id=product_id).exists():
                return Response({'error':"Product wish this ID does not exist"},status=status.HTTP_404_NOT_FOUND)
            product=Product.objects.get(id=product_id)
